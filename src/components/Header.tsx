...
export function Header() {
    ...
    return (
        <header ref={outletRef}
                className={`min-h-42 w-screen flex justify-between items-center px-13 py-6 ${active ? "bg-avit-grey shadow-lg" : ""}`}>
            ...
            <div className="flex items-center justify-end">
                {active && <button onClick={() => showModal('none')}
                    ...
                    <Icon icon="material-symbols:home-outline-rounded" width={56} height={56}/>
                    <div className="text-xl font-semibold">Home</div>
                </button>}
                {active && <button onClick={() => showModal('tour')}
                    ...
                    <Icon icon="material-symbols:explore-outline-rounded" width={56} height={56}/>
                    <span className="text-xl font-semibold">Tour</span>
                </button>}
                <button onClick={() => showModal('support')}
                ...
                {active && <button onClick={() => showModal('shutdown')}
                    ...
                    <Icon icon="material-symbols:cancel-outline" width={56} height={56}/>
                    <span className="text-xl font-semibold">End</span>
                </button>}
            </div>
            {modalType == 'support' && <SupportModal onClose={() => closeModal()}/>}
            {modalType == 'settings' && <SettingsModal initialTab={initialTab} onClose={() => closeModal()}/>}
